...
def normalize_for_asr_metric(text: str) -> str:
    ...
    normalized = re.sub(r"\b3\s*초", "세 초", normalized)
    normalized = re.sub(r"\b6\s*시", "여섯 시", normalized)
    normalized = re.sub(r"\b30\s*분", "삼십 분", normalized)
    normalized = normalized.replace("세초", "세 초")
    normalized = normalized.replace("따라할", "따라 할")
    normalized = normalized.replace("한가지", "한 가지")
    normalized = normalized.replace("꼬치는", "꽂히는")
