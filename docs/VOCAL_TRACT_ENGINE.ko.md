...
`dinosaur_giant`와 `dinosaur_giant_roar`는 이제 사람 목소리가 들리는 voice conversion이 아니라 bioacoustic v3 renderer를 사용합니다.

- 원본 화자의 들리는 정체성은 제거
- 원본 녹음은 길이, 에너지 곡선, 강약 변화 제어에만 사용
- 출력은 낮은 closed-mouth boom, 몸통 울림, throat grit, pressure noise로 합성

이전 공룡 v2 레이어 체인은 fallback/debug 경로로만 남깁니다. 제품 경로에서는 다이노사우르스 목소리에 사람 말소리 힌트가 남지 않도록 합니다.
